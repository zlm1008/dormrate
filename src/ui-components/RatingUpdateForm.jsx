...
export default function RatingUpdateForm(props) {
  ...
  const resetStateValues = () => {
    ...
    setUniversityRating(cleanValues.UniversityRating);
    setDormName(cleanValues.DormName);
    setDormRating(cleanValues.DormRating);
    setRoomNumber(cleanValues.RoomNumber);
    setRoomRating(cleanValues.RoomRating);
  ...
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Rating0, idProp)
        : ratingModelProp;
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Rating0.copyOf(ratingRecord, (updated) => {
